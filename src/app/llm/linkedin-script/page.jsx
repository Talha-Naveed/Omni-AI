...
export default function Home() {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      setTranscriptGenerated(
        `Generated transcript for LinkedIn profile: ${url}`
      );
      setLoadingMessage("");
      setShowModal(true);
    }, 3000);
  ...
  return (
    <div className="min-h-screen bg-[#1E1B31] text-white flex flex-col items-center justify-center p-6">
      {loadingMessage && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-10 text-xl">
          {loadingMessage}
        </div>
      )}
      ...
      <p className="text-xl text-gray-300 mb-6">
        Enter a LinkedIn URL to generate a transcript.
      </p>
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-lg space-y-6">
        ...
        <button
          type="submit"
          className="w-full py-3 bg-gradient-to-r from-pink-500 to-red-500 text-white font-semibold rounded-lg"
        >
          Generate Transcript
        </button>
      </form>
      ...
      {showModal && (
        ...
        >
          <div
            className="bg-white text-black p-6 w-96 rounded-lg shadow-lg"
          ...
          >
            ...
            <button
              onClick={() => setShowModal(false)}
              className="mt-6 py-2 px-4 bg-red-500 text-white rounded-lg"
            >
              Close
            </button>
          </div>
